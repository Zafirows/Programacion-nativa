...
// Importa los recursos (strings e imágenes) y el modelo Sport
import com.example.sports.R
import com.example.sports.model.Sport

/**
 * Proveedor de datos local para la app de deportes
 */
object LocalSportsDataProvider {

    // Deporte por defecto (el primero en la lista)
    val defaultSport = getSportsData()[0]

    // Función que devuelve una lista de deportes predefinidos
    fun getSportsData(): List<Sport> {
        return listOf(
            // Cada deporte se define con sus propiedades y recursos asociados

            Sport(
                id = 1,
                titleResourceId = R.string.baseball, // Título del deporte (Baseball)
                subtitleResourceId = R.string.sports_list_subtitle, // Subtítulo común
                playerCount = 9, // Número de jugadores
                olympic = true, // ¿Es deporte olímpico?
                imageResourceId = R.drawable.ic_baseball_square, // Imagen cuadrada
                sportsImageBanner = R.drawable.ic_baseball_banner, // Banner del deporte
                sportDetails = R.string.sport_detail_text // Texto de detalle
            ),
            Sport(
                id = 2,
